...
import { setup as telegramSetup, ProtoTelegraf } from "./telegram2discord/setup";

// Discord stuff
import { Client as DiscordClient, Intents } from "discord.js";
import { setup as discordSetup } from "./discord2telegram/setup";
import { ProtoCrossContext } from "./telegram2discord/endwares";
...
if (R.not(R.equals(rawSettingsObj, newRawSettingsObj))) {
	...
	try {
		...
		if (err.code === "EACCES") {
			...
			logger.warn(
				"Setting file is not accessible"
			);

			// Write the settings to temp instead
			const tmpPath = path.join(os.tmpdir(), "protobridge-settings.yaml");
			try {
...
telegramSetup(logger, tgBot as ProtoTelegraf, dcBot, messageMap, bridgeMap, settings);
