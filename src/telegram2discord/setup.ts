...
import { chatinfo, handleEdits, leftChatMember, newChatMembers, relayMessage, ProtoCrossContext } from "./endwares";
...
export interface ProtoTelegraf extends Telegraf {
	use: any | ProtoCrossContext;
	// eslint-disable-next-line
	on: any | ((value: string) => ProtoCrossContext);
	context: ProtoCrossContext;
}
...
export function setup(
	...
	tgBot.ready = Promise.all([
		...
		.then(([me]) => {
			...
			tgBot.use(middlewares.addProtoVerseCrossObj);
			...
			//@ts-ignore telegram expects a second parameter
