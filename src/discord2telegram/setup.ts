...
const waterMarkText = (text: string) => {
	const waterMark = "\n\n<b>Powered by @ProtoVerse_ai</b>";
	...
	return function (member: any) {
		...
		member.guild.channels.cache
			...
			.forEach(async (bridge: any) => {
				...
				try {
					// Send it
					await tgBot.telegram.sendMessage(bridge.telegram.chatId, waterMarkText(text), {
						parse_mode: "HTML"
					});
				} catch (err) {
...
export function setup(
		...
		if (!R.isEmpty(bridges)) {
			bridges.forEach(async bridge => {
				...
				message.attachments.forEach(async ({ url }) => {
					try {
						const textToSend = bridge.discord.sendUsernames
							? `<b>${senderName}</b>\n<a href="${url}">${url}</a>`
							: `<a href="${url}">${url}</a>`;
						const tgMessage = await tgBot.telegram.sendMessage(bridge.telegram.chatId, waterMarkText(textToSend), {
							parse_mode: "HTML"
						});
						messageMap.insert(
				...
				message.embeds.forEach(embed => {
					...
					try {
						// Send it
						tgBot.telegram.sendMessage(bridge.telegram.chatId, waterMarkText(text), {
							parse_mode: "HTML",
				...
				if (message.cleanContent) {
					...
					try {
						var textToSend = bridge.discord.sendUsernames
							? `<b>${senderName}</b>\n${processedMessage}`
							: processedMessage;
	...
	dcBot.on("error", (err: Error) => {
		...
		if (err.code === "ECONNRESET") {
			...
			logger.warn(
				"Lost connection to Discord's servers. The bot will resume when connection is reestablished."
			);
		} else {
			// Unknown error. Tell the user to tell the devs
			logger.error(
				"The Discord bot ran into an error."
			);
			logger.error(err);
		}
	});
